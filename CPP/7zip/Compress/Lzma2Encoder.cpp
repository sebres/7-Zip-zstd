...
HRESULT CFastEncoder::FastLzma2::SetCoderProperties(const PROPID *propIDs, const PROPVARIANT *coderProps, UInt32 numProps)
{
  ...
  dictSize = Z7_min(dictSize, reduceSize);
  dictSize = Z7_max(dictSize, (size_t)FL2_DICTSIZE_MIN);
  ...
  if (lzma2Props.blockSize == 0)
    lzma2Props.blockSize = Z7_min(Z7_max(MIN_BLOCK_SIZE, dictSize * 4U), MAX_BLOCK_SIZE);
